...
This is not the venue for any comparative analysis on the merits of Go over any other language. I like them all. I happen to have simply chosen Go (as well as Angular, and Postgresql) as key pieces for my own projects. I like that Go generates a stand-alone executable without the need for bulky confusing libraries. Deploying a Go executable is simply copy-and-paste. With Go you can easily cross-compile for another CPU/OS combination.

The [Go wiki](https://github.com/golang/go/wiki) has a page entitled [Why Go](https://github.com/golang/go/wiki/whygo). Here is a wiki page on [Go Users](https://github.com/golang/go/wiki/GoUsers). Here is another that discusses [moving a code base to Go](https://github.com/golang/go/wiki/FromXToGo) from some other language. And here is a page on Go [success stories](https://github.com/golang/go/wiki/SuccessStories) from around the web.

One of the first implementation of [Ethereum is in Go](https://geth.ethereum.org/). Go is also the preferred language for [Blockchain](https://medium.com/@pkthakur01/why-golang-for-blockchain-40f874f2ce1b). The Chrome [download](https://www.google.com/chrome/downloads/) website runs Go as told by these [slides](https://talks.golang.org/2013/oscon-dl.slide#1).

Docker and its toolset (swarm, engine, compose, and etc.) are all written in Go and so are Kubernetes, rclone, gocryptfs, Hugo, etcd, Syncthing, CockroachDB, LXD, RQLite, DQLite, and many more. The reference code for [gRPC](https://grpc.io/) and [Raft](https://raft.github.io/) Consensus Algorithm are in Go.

When you order online from Walmart or Target, you are browsing Go websites. Or when you take Uber or Lyft, these companies are Go users. Go software is [everywhere now](https://github.com/avelino/awesome-go).
...
Installing Go is straightforward. Follow the following steps.
